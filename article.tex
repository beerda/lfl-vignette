...
\author{Michal Burda\\University of Ostrava
   \And Martin \v{S}t\v{e}pni\v{c}ka\\University of Ostrava}
\Plainauthor{Michal Burda, Martin \v{S}t\v{e}pni\v{c}ka}

%% - \title{} in title case
%% - \Plaintitle{} without LaTeX markup (if any)
%% - \Shorttitle{} with LaTeX markup (if any), used as running title
\title{\pkg{lfl}: Linguistic Fuzzy Logic in \R{}}
\Plaintitle{lfl: Linguistic Fuzzy logic in R}
\Shorttitle{\pkg{lfl}: Linguistic Fuzzy Logic in \R{}}

%% - \Abstract{} almost as usual
\Abstract{
  This paper presents an \R{} package that enables the use of {\color{blue} fuzzy relational calculus and} linguistic
  fuzzy logic in data processing applications. The \pkg{lfl} package {\color{blue}enables computing compositions of fuzzy relations enhanced with distinct extensions, such as excluding features, unavoidable features, or generalized quantifiers.} {\color{blue}Furthermore, it} provides tools for transformation of data into fuzzy sets representing linguistic expressions, mining
  of linguistic fuzzy association rules, performing an inference on fuzzy rule bases
  using the Perception-based logical deduction (PbLD). The package also enables to use the Fuzzy rule-based ensemble,
...
\Address{
  Michal Burda,  Martin \v{S}t\v{e}pni\v{c}ka\\
  ...
  E-mail: \email{michal.burda@osu.cz}, \email{martin.stepnicka@osu.cz}\\
  URL: \url{https://ifm.osu.eu/}, \url{https://ifm.osu.eu/michal-burda/33404/}, \url{https://ifm.osu.eu/martin-stepnicka/366/}
}
...
However, it is not connected with the \R{} statistical environment and unlike \pkg{lfl} package, LFLC is not fully free nor an open-source.For an exhaustive study on existing software implementations of fuzzy methods we refer to the recent survey \cite{AlcalAlonso:fuzzySW_IEEETFS16}.


The \pkg{lfl} package also provides functions for searching for fuzzy association rules (\cite{srikant95}). Together
...
Based on the selected t-norm, the \pkg{lfl} package provides all the derived operations in a concise and extendable way. By calling the \code{algebra()} function with the name of the underlying t-norm, an instance of the S3 \code{algebra} class is obtained, which is a named list of functions. The user may select from \code{"goedel"}, \code{"goguen"}, or \code{"lukasiewicz"} variant calling the respective G\"odel, Goguen (also often called \emph{product}), or the already above-mentioned \L ukasiewicz residuated lattices of operations. 

The instances of the \code{algebra} class serve often as a parameter to many other functions of the \pkg{lfl} package. A user may extend these objects by selecting from some predefined missing value handling schemes (see Section~\ref{sec:undefvals}) or by defining a custom algebra.
...
\begin{itemize}
    ...
    \item \code{order}: {\color{green} ordering of truth values that always put \code{NA} values in the end.}
\end{itemize}
...
\begin{itemize}
    ...
    \item \code{s}, \code{ps}: vectorized and element-wise supremum defined as:
    $\alpha \lor \beta = \max\{\alpha, \beta\};$
    \item \code{order}: {\color{green} ordering of truth values that always puts \code{NA} values in the end.
    }
\end{itemize}
...
Sobocinski and Bochvar approaches with the preservation of the ordering $0 \le \texttt{NA} \le 1$ which ,on the other hand, reflects some aspects of the Kleene algebra. The distinct algebraic incorporation of the treatment of missing values is provided in Tables~\ref{tab:res-negation}-\ref{tab:equivalences}.
...
\michal{tohle je fajn popis, ale ta \code{order} funkce pro algebry je porad jeste work-in-progress, protoze neni uplne jasne, jak ma fungovat pro jine algebry nez je bochvar, dragonfly a lowerEst. Je potrebna pro kvantifikatory a tam to jeste neni uplne jasne jak to delat, takze bych mozna tohle presel v tichosti, v Rku pri pokusu o pouziti kvantifikatoru zarve hlaska ze to jeste neumime a snad tento odstavec muzeme prozatim vynechat.}{\color{red}Chapu snahu se tomu vyhnout, ale moc tomu popisu nerozumim. Pro dve algebry (D a Le) to uz umime, pro treti algerbu (B) si myslime, ze to umime. Budes to mit deaktivovane pro vsechny vcetne techto dvou ci tri? A proc pak nahore uvadis o kterekoliv underlying algebry order: puts NA at the end? To preci potrebujes jen pro ty kvantifikatory a nyni tu pises, ze Rko pri pokusu o pouziti kvantifikatoru zarve chybova hlaska. A v clanku, kde jsme to delali pro D, se na R package lfl odkazujeme, takze to uplne vypnute asi mit nemame. Nicmene bude-li to vypnute, proc pak popisujeme vyse ten order?}

\michal{1. takže jak to je v současné době v kódu: kvantifikátory fungují pro bochvara (default), dragonfly a lowerest. pro všechno ostatní to zařve chybovou hlášku, že není implementována funkce \code{order}.

2. proč tam nahoře popisuju tu funkci order: je to proto, že tam je, že se zobrazuje v tom generovaném výpisu. spravne by to melo byt popsane, protoze to je dulezita soucast te algebry, bez ktere by to nefungovalo a kdyby treba nekdo chtel vyrobit dalsi, svou algebru, tak potrebuje vedet, ze tam je nejaky ten order. na druhou stranu, pro nektere algebry docasne nevime, co tam ma byt. trochu se v tom placam, jak vidis a nevim kudy kam. nejradsi bych sedl a ten order udelal i pro ostatni algebry. anebo zrusime ten vypis funkci jednotlivych algeber a jen popiseme ty klasicke operace a order vynechame uplne?}

As mentioned above, each algebra for partial fuzzy logic adopts its partial
ordering on the extended set of truth-values $[0,1]\cup \texttt{NA}$. For instance, \code{NA} is incomparable to any $\alpha\in [0,1]$ for the Soboci\'{n}ski or Bochvar algebra, while the same holds only for $\alpha\in (0,1)$ for the Kleene, Dragonfly or Lower Estimation algebra with the additional property $0\le\texttt{NA}\le 1$. However, for the last two algebra, there is additional ``lattice-like'' ordering  $ \leq_{\ell} $ generated by the  lattice operations meet and join (conjunction, disjunction), that is defined as follows: $ 0\leq_{\ell}\texttt{NA}\leq_{\ell}\alpha $ for all $ \alpha\in (0, 1] $, see  \cite{Step_etal_Dragon_IJAR2019}. 
...
The use of the compositions may be easily demonstrated on a toy example from medical diagnosis that by purposes simplifies the situation for the sake of clarity. 
...
%
Additional examples of more complicated compositions computed with the \code{mult()} function may be found below in Section~\ref{sec:compquant}.
...
For example, let us imagine that the occurrence of fever is an evidence of NOT having a pulmonary hypertension. Similarly, let cough excludes hangover from the possible diagnoses. That is, fever and cough are excluding features of pulmonary hypertension and hangover, respectively. This corresponds to the following matrix $E$ of excluding features:
%
\input{chunks/chunk-comp.e}
%
To compute $\RSE$ as in (\ref{form:circ_excl}), one can proceed as follows:
%
...
%
$(R\circ S)^{\rhd U}$ (see (\ref{form:circ_unavoid})) may be evaluated by the following commands: 
%
\input{chunks/chunk-comp.circ.unavoid}
%
Finally, the concept unavoidable and excluding features together, as defined in (\ref{form:circ_excl_unavoid}), is processed as follows:
%
...
Another approach to avoid the undesirable effect of too many suspicions (classification candidates) provided by the basic composition and too few (or often none)  suspicions provided by the Bandler-Kohout products is based on employing generalized quantifiers. {\color{blue}Intermediate generalized quantifiers allow to deal with quantifications in between of the classical universal and existential quantifiers, for example {\tt Most}, {\tt Many}, {\tt At least 3}, {\tt At least 25\%}, or {\tt A few}.}

The construction of a {\color{blue}quantifier $Q$ of the type $\langle 1 \rangle$  \citep{dvorak2009} uses a symmetric fuzzy measure $\mu$, i.e. a monotone measure on the potential set satisfying the boundary condition. The direct application of the quantifier to the compositions, i.e. $ R @^{Q} S $ where $ @\in \{\circ,\lhd \} $, has been proposed in \cite{Step2014,CaoStepHolc_Quant:EUSFLAT2015}  and} it is defined as follows:
    \begin{align}
...
Due to the choice of the symmetric fuzzy measure $\mu$, it is sufficient to consider relative cardinality and its modification by distinct non-decreasing functions $f$ in order to use the  Sugeno-integral \citep{KlementMesiarPap_IEEE2010}: 
\begin{align}
(R@^{Q} S) (x,z) =  \bigvee_{i=1}^n \left( \left( R(x,y_{\pi(i)})
\circledast S(y_{\pi(i)}, z) \right) \otimes f(i/n) \right)
\label{form_calcu_@}
\end{align}
...
$n$ denotes the cardinality of $Y$.


The \pkg{lfl} package provides a function for computation of the Sugeno integral, which can be used for composition of fuzzy relations with the \code{mult()} function described in Section~\ref{sec:fundcomp}.

For instance, one may require the patients to show at least two symptoms of the diagnosis. To quantify the ``at least 2'' condition over a fuzzy set, a Sugeno integral will be applied. For that, we need a \code{measure} function that returns a truth value from the $[0,1]$ interval. For us, a simple conditional function will do the work:
%
\input{chunks/chunk-comp.quant.atleast}

The \code{sugeno()} function requires four arguments: \code{measure}, \code{relative}, \code{strong} and \code{alg}. The \code{measure} argument is a non-decreasing  function that assigns a truth value from the $[0, 1]$ interval to either relative or absolute quantity. The \code{relative} argument is a \code{TRUE}/\code{FALSE} flag indicating what is expected by the \code{measure} function. The \code{strong} argument determines whether {\color{red}$\otimes$ (bylo tu $*$ asi diky me chybe vyse)} is a strong or weak conjunction. Finally, the \code{alg} argument is an underlying algebra, i.e., either a string or object of type \code{algebra}.

The \code{sugeno()} function creates a Sugeno-integral computer, which is a two-argument function that expects two numeric vectors of equal length: the first argument, \code{x}, is a vector of membership degrees to be measured, while the second argument, \code{w}, is a vector of weights.
...
%
The \code{specs} attribute returns a matrix that encodes a sort of \emph{specificity} relation see Section~\ref{sec:pbld}) between the columns of the \code{fsets} object (some columns and rows are omitted for brevity):
%
...
%
\input{chunks/chunk-fcut.logfact}
%
...
Additionally, combined fuzzy sets may be created by using the argument \code{merge}. If \code{merge=1} (default), only the elementary fuzzy sets discussed above are produced. 
...
The \code{merge} argument may contain multiple values. In that case, all types of merged fuzzy sets is provided {\color{red} (is?)}. In the following example, the \code{merge} argument is a numeric vector containing \code{1}, i.e., the elementary fuzzy sets are created. It also contains \code{2}, hence the combinations of two consecutive elementary fuzzy sets is provided too {\color{red} (is?)}. Finally, as also \code{3} is present in \code{merge}, a single fuzzy set that combines all the three elementary fuzzy sets is returned as well:
%
...
\begin{itemize}
    ...
    \item \code{rhs} -- indices of data columns that may appear on the left-hand side of the rule (i.e., in the consequent);
    ...
    \item \code{numThreads} -- the number of computing threads to start in the parallel computation.
\end{itemize}
...
%
\input{chunks/chunk-searchrules2}
%
...
also another variant, see e.g. \cite{DvoStep:PbLD2015}. The first one can be called \emph{global PbLD} and the latter one as \emph{local PbLD}. The names, in the \proglang{R}-package implementation mirrored in the arguments \code{global} and \code{local}, point to the application of the specificity ordering in the perception function that chooses particular rules to be fired. Unless explicitly stated, all what is described below has a general validity for both variants.
...
%
Now we split the data into the \code{training} and \code{testing} part. For that, here are 10 data rows randomly selected for the testing dataset (in real application, perhaps more sophisticated method of training/testing split may be performed e.g. by using the \code{createDataPartition()} function of the \pkg{caret} package \citep{caretPkg}):
%
...
The \code{perceive()} function handles the perception, which is a central notion of the PbLD inference. From a set of rules, \code{perceive()} removes each rule for which another rule exists that is more specific. The specificity is determined by calling the \code{is.specific()} function. In other words, for each rule $\mathcal{R}_i$ in the list of rules, it searches for another rule $\mathcal{R}_j$ such that \code{is.specific(Rj, Ri, ...)} returns \code{TRUE}. {\color{red}lepe: ``If the answer is positive then ...''}If yes then $\mathcal{R}_i$ is removed from the list. The specificity is based on the \code{specs()} matrix defined as an attribute of the \code{fsets} object, which can be created by the \code{lcut()} or \code{fcut()} function.

The \code{defuzz()} function performs the defuzzification. As the character of the outputs obtained by the (\ref{form:PBLDconclusion}) leads to expressions that look like modified evaluative linguistic expressions, the use of the \emph{Defuzzification of Evaluative Expressions} (DEE) that has been designed for these purposes seems reasonable. It is a hierarchical two-step defuzzification that firstly classifies the type of the output  into three types: S$^-$ ({\tt Small}), $\Pi$ ({\tt Medium}), and S$^+$ ({\tt Big}). This step is done based on the monotonicity of the output fuzzy set membership functions, e.g., non-increasing is  classified as S$^-$ ({\tt Small}). In its second step, DEE calls one of the standard defuzzifications for implicative rules, \emph{First-Of-Maxima} (FOM) is used for the S$^-$ type,
...
This article presents the \code{R}-package \pkg{lfl} that provides tools and functions for fuzzy relational calculus and fuzzy natural logic. It provides the implementation of the most usual algebras of operations on fuzzy sets (G\"{o}del, Goguen, and \L ukasiewicz), rich variety of compositions of binary fuzzy relations, namely, the basic (circlet) one, all three Bandler-Kohout products, extensions with more binary fuzzy relations and generalized quantifiers. Of course, their combinations are allowed as well. The chosen algebra always holds the position of an argument so, all the compositions may be calculated in an of the chosen function and thus, all the implemented functions can run on any of the algebra of functions. Furthermore, in order to cover various situations when the membership degree is only partially defined  (inconsistency, undefinedness, missing values), distinct algebras for partial fuzzy logics, namely Bohchvar, Soboci\'{n}ski, Kleene, Nelson, Deagonfly, and Lower estimation algebras are implemented as well.  
 ...
 The fuzzy rules cab be defined expertly as well as learned from data -- using the associations mining. The \pkg{lfl} package contains original method of mining associations rules with the evaluative expressions in antecedents as well as consequents. Automatic redundancy analysis is supplied with a size reduction algorithms that additionally reduces the size in cases when the deletion of redundant rules is not sufficient. 
